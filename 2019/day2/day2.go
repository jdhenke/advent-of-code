...
func Part1(r io.Reader) (answer int, err error) {
	code, err := intcode.Parse(r)
	if err != nil {
		return 0, err
	}
	return day2(code, 12, 2), nil
}
...
func Part2(r io.Reader) (answer int, err error) {
	...
	for noun := 0; noun < 100; noun++ {
		for verb := 0; verb < 100; verb++ {
			cp := make(intcode.Code, len(code))
			copy(cp, code)
			if day2(cp, noun, verb) == 19690720 {
				return 100*noun + verb, nil
			}
		}
	}
	return 0, fmt.Errorf("did not find answer")
}

func day2(code intcode.Code, noun, verb int) int {
	code[1] = noun
	code[2] = verb
	return intcode.Run(code)
}
