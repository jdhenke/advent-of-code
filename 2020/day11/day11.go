...
func Part1(r io.Reader) (answer int, err error) {
	return day11(r, 4, func(g input.Grid, i, j int) int {
		occ := 0
		g.ForEachAdj(i, j, func(s string) {
			if s == "#" {
				occ++
			}
		})
...
func Part2(r io.Reader) (answer int, err error) {
	return day11(r, 5, part2GetOccupied)
}

func part2GetOccupied(g input.Grid, i, j int) int {
	occ := 0
...
func day11(r io.Reader, threshold int, getOccupied func(g input.Grid, i, j int) int) (answer int, err error) {
	g, err := input.NewGrid(r)
	if err != nil {
		return
	}
	for {
		...
		changes := make(map[entry]string)
		g.ForEach(func(i, j int, s string) {
			occ := getOccupied(g, i, j)
			switch g.Get(i, j) {
			case "L":
				if occ == 0 {
					changes[entry{i, j}] = "#"
