...
func day17(r io.Reader) (answer int, err error) {
	...
	for step := 0; step < 6; step++ {
		changes := make(map[[3]int]bool)
		checked := make(map[[3]int]bool)
		for x := range grid {
			if n := activeNeighbors(grid, x); n != 2 && n != 3 {
				changes[x] = false
			}
			forEachNeighbor(grid, x, func(y [3]int) {
				if grid[y] {
					return
				}
				...
				if n := activeNeighbors(grid, y); n == 3 {
					changes[y] = true
				}
				checked[y] = true
			})
...
func activeNeighbors(grid map[[3]int]bool, x [3]int) int {
	active := 0
	forEachNeighbor(grid, x, func(y [3]int) {
		if grid[y] {
			active++
...
func forEachNeighbor(grid map[[3]int]bool, x [3]int, f func(y [3]int)) {
	for i := -1; i <= 1; i++ {
		for j := -1; j <= 1; j++ {
			for k := -1; k <= 1; k++ {
				if i == 0 && j == 0 && k == 0 {
