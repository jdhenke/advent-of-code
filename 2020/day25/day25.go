...
func day25(r io.Reader) (answer int, err error) {
	...
	publicKey1, err := strconv.Atoi(string(lines[0]))
	if err != nil {
		return 0, err
	}
	publicKey2, err := strconv.Atoi(string(lines[1]))
	if err != nil {
		return 0, err
	}

	// brute force loop number for public key 1
	loopNum1, ok := findLoopNum(publicKey1)
	if !ok {
		return 0, fmt.Errorf("could not find loop number for private key 1")
	}

	// create encryption key by encrypting public key 2 using loop number 1
	return loop(publicKey2, loopNum1), nil
}

func loop(sn, loopNum int) int {
	val := 1
	for i := 0; i < loopNum; i++ {
		val *= sn
		val %= 20201227
...
func findLoopNum(publicKey int) (loopNum int, ok bool) {
	val := 1
	const sn = 7
	for i := 0; i < 20201227; i++ {
		val *= sn
		val %= 20201227
		if val == publicKey {
			return i + 1, true
