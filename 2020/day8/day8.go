...
func Part2(r io.Reader) (answer int, err error) {
	...
	for i := 0; i < len(cmds); i++ {
		...
		t := cmd
		t.op = map[string]string{
			"jmp": "nop",
			"nop": "jmp",
		}[cmd.op]
		cmds[i] = t
		if ans, ok := run(cmds); ok {
			return ans, nil
		}
		cmds[i] = cmd
	}
...
func run(cmds []command) (answer int, ok bool) {
	var accumulator int
	seen := make(map[int]bool)
	i := 0
	for !seen[i] && i < len(cmds) {
		seen[i] = true
		cmd := cmds[i]
		switch cmd.op {
		case "acc":
...
func parseCommands(r io.Reader) ([]command, error) {
	...
	if err := input.ForEachLine(r, func(line string) error {
		...
		if err != nil {
			return fmt.Errorf("bad line '%s': %v", line, err)
		}
