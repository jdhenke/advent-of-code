...
func day1(r io.Reader, windowSize int) (answer int, err error) {
	var (
		i         = 0
		recent    = make([]int, windowSize)
		increases = 0
	)
	if err := input.ForEachInt(r, func(x int) {
		i++
		// The sum of the previous window and the current window will
		// be made of the same values except the previous sum drops the
		// (i - windowSize)^th value and the current sum adds the i^th
		// value, so to tell if the sum increased or decreased we need
		// only compare those two.
		if i > windowSize && x > recent[i%windowSize] {
			increases++
		}
		recent[i%windowSize] = x
	}); err != nil {
		return 0, err
	}
